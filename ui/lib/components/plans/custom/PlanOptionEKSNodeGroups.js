            ...
            <>
              ...
              <Collapse defaultActiveKey={['basics','compute','metadata']}>
                ...
                <Collapse.Panel key="compute" header="Compute Configuration (instance type, GPU or regular workload)">
                  ...
                  <PlanOption {...this.props} forceShow={showAll} id={`${id_prefix}_instanceType`} displayName="Instance Root Disk Size (GiB)" name={`${name}[${selectedIndex}].diskSize`} property={property.items.properties.diskSize} value={selected.diskSize} editable={this.isEditable(property.items.properties.diskSize)} onChange={(_, v) => this.setNodeGroupProperty(selectedIndex, 'diskSize', v)} />
                </Collapse.Panel>
